...
function linkOrCopy(src, dst, { force }) {
  ...
  // Prefer copy over symlink: many IDEs (Claude Code on Windows in particular)
  // don't follow symlinks reliably inside their commands/prompts dirs.
  // Trade-off: user needs to re-run `install` to pick up upstream changes.
