...
export function install(ctx) {
  ...
  const baseCommand = pythonCommandString(python, HOOK_SCRIPT);
  const perTarget = [];

  for (const t of targets) {
    const command = t.hookCommandExtraArgs && t.hookCommandExtraArgs.length
      ? `${baseCommand} ${t.hookCommandExtraArgs.map((a) => (a.includes(" ") ? `"${a}"` : a)).join(" ")}`
      : baseCommand;
  ...
  return { ok: true, python, command: baseCommand, targets: perTarget, dryRun };
}
